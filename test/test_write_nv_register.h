...
int test_write_nv_register(void)
{
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        {
            fprintf(stderr, "[test_write_nv_register:%d] unexpected error on init: %d\n", i, ret);
            return 1;
        }
        ...
        if (ret != MAX5494_MAX5499_OK)
        {
            fprintf(stderr, "[test_write_nv_register:%d] unexpected error on write_nv_register: %d\n", i, ret);
            return 1;
        }

        if (spi.length != 3)
        {
            fprintf(stderr, "[test_write_nv_register:%d] length expected: 3, actual: %d\n", i, spi.length);
            return 1;
        }
...
int test_write_nv_register_propagates_error(void)
{
    typedef struct test_case_t
    {
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        if (ret != MAX5494_MAX5499_OK)
        {
            fprintf(stderr, "[test_write_nv_register_propagates_error:%d] unexpected error on init: %d\n", i, ret);
            return 1;
        }

        ret = max5494_max5499_write_nv_register(&rdac, MAX5494_MAX5499_RA0, 0);
        if (ret != cases[i].expected)
        {
            fprintf(stderr, "[test_write_nv_register_propagates_error:%d] expected: %d, actual: %d\n", i, cases[i].expected, ret);
            return 1;
        }
    }
